...
export const ApiKeyModal: React.FC<ApiKeyModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div className={`border rounded-xl w-full max-w-lg shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 transition-colors ${
        ...
        <div className={`flex items-center justify-between p-6 border-b ${isDark ? 'border-zinc-800' : 'border-zinc-200'}`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-semibold tracking-tight">FortyGuard API & Settings</h2>
              <p className={`text-sm ${isDark ? 'text-zinc-400' : 'text-zinc-500'}`}>Configure Live Enterprise API or Demo Simulator</p>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="p-6 space-y-4">
          ...
          <div className={`p-4 border rounded-lg flex items-start gap-3 ${
            ...
            <div className="flex-1 text-sm">
              <div className="flex items-center justify-between font-medium">
                <span>Active Mode: {systemStatus?.mode === 'live' ? 'Live FortyGuard API' : 'Demo Simulator'}</span>
                <span className={`text-xs font-mono ${isDark ? 'text-zinc-400' : 'text-zinc-500'}`}>{systemStatus?.plan_tier}</span>
              </div>
          ...
          {/* Mode Selection */}
          <div>
            <label className={`block text-xs font-semibold uppercase tracking-wider mb-2 ${isDark ? 'text-zinc-400' : 'text-zinc-500'}`}>
              Execution Mode
            </label>
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className={`p-3.5 rounded-lg border text-left transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500/40 ${
                  selectedMode === 'demo'
              ...
              >
                ...
                <p className={`text-xs mt-1 ${isDark ? 'text-zinc-400' : 'text-zinc-500'}`}>Realistic synthetic microclimate telemetry without key.</p>
              </button>
              ...
              >
                ...
                <p className={`text-xs mt-1 ${isDark ? 'text-zinc-400' : 'text-zinc-500'}`}>Connects directly to FortyGuard Enterprise v1.</p>
              </button>
            </div>
          </div>
          ...
          <div>
            <label className={`block text-xs font-semibold uppercase tracking-wider mb-1.5 ${isDark ? 'text-zinc-400' : 'text-zinc-500'}`}>
              FortyGuard API Key {selectedMode === 'demo' && <span className="font-normal opacity-70">(Optional for Demo)</span>}
            </label>
            <input
              type="password"
              value={apiKeyInput}
              ...
              className={`w-full border rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/40 transition-colors font-mono ${
                isDark 
          ...
          {feedback && (
            <div className={`p-3 rounded-lg text-sm flex items-center gap-2 ${
              ...
              {feedback.type === 'success' ? <CheckCircle2 className="w-4 h-4 text-green-500" /> : <AlertTriangle className="w-4 h-4 text-red-500" />}
        ...
        {/* Footer */}
        <div className={`p-6 border-t flex items-center justify-end gap-3 ${isDark ? 'border-zinc-800' : 'border-zinc-200'}`}>
          <button
            onClick={onClose}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500/40 ${
          ...
          <button
            onClick={handleSave}
            disabled={isLoading}
            className="flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-medium bg-blue-500 hover:bg-blue-600 text-white transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500/40 disabled:opacity-50"
          >
